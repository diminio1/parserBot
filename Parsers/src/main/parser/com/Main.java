...
public class Main {
	...
	//			  System.err.println("Exception: " + ex.getMessage());
			...
			if (connCountryCityStand != null){
			
				selectCountry = connCountryCityStand.createStatement();		
				countryResult = selectCountry.executeQuery("select * from COUNTRY;");   
				
				countryStand = new TermFilter(createBaseStand(countryResult, 0));
				
				selectCity = connCountryCityStand.createStatement();		
    ...
    private static void setToutToCountryConn (ArrayList <TourObject> tours, int tourId, PreparedStatement select, int i, int tourNumber) throws SQLException{
    	...
    	if(tours != null){
	   		...
	   		for (int count = 0; count < tours.get(i).country.size(); count ++) {
	   			...
	   			select = conn.prepareStatement("INSERT INTO TOURS_TO_COUNTRIES VALUES (" +
						       					 intResultCountryConNum + ", " +  
    ...
    private static void setTourToCityConn (ArrayList <TourObject> tours, int tourId, PreparedStatement select, int i, int tourNumber) throws SQLException{
    	...
    	if(tours != null){
       		...
       		for (int count = 0; count < tours.get(i).town.size(); count ++) {
       			...
       			select = conn.prepareStatement("INSERT INTO TOURS_TO_CITIES VALUES (" +
       					 intResultCityConNum + ", " +  
